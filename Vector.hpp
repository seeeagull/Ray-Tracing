...
inline Vector3f rotateY(const Vector3f &a, float theta)
{
    float w1 = cos(theta), w2 = sin(theta);
    Vector3f b = Vector3f(w1 * a.x + w2 * a.z, a.y, w2 * a.x - w1 * a.z);
    return b;
}

inline Vector3f rotateZ(const Vector3f &a, float theta)
{
    float w1 = cos(theta), w2 = sin(theta);
    Vector3f b = Vector3f(w1 * a.x - w2 * a.y, w2 * a.x + w1 * a.y, a.z);
    return b;
}

inline Vector3f rotate(const Vector3f &a, float theta, float alpha)
{
    float theta_ori = acos(a.z), alpha_ori = atan(a.y / (a.x + 1e-5));
    Vector3f b = Vector3f(a.x, a.y, a.z);
    b = rotateZ(b, -alpha_ori);
    b = rotateY(b, -theta_ori);
    b = rotateY(b, theta);
    b = rotateZ(b, alpha);
    b = rotateY(b, theta_ori);
    b = rotateZ(b, alpha_ori);
